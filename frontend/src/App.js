...
import { MagnifyingGlassIcon, HeartIcon, MoonIcon, SunIcon, XMarkIcon } from '@heroicons/react/24/solid';
...
if (!finalQuery.trim()) return;
  ...
  const toggleFavorite = (index, item) => {
    ...
    if (updatedFavorites[index]) {
      delete updatedFavorites[index]; // Remove from favorites
    } else {
      updatedFavorites[index] = item; // Add to favorites
    }
    ...
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gradient-to-br from-zinc-900 to-black text-zinc-100' : 'bg-white text-zinc-900'} pb-24 transition-colors duration-300`}>
      ...
      <nav className={`px-6 py-4 flex justify-between items-center shadow-lg sticky top-0 z-10 border-b ${theme === 'dark' ? 'bg-zinc-800 text-white border-zinc-700' : 'bg-white text-black border-zinc-200'}`}>
        ...
        <div className="flex items-center space-x-4 text-sm">
          ...
          <button
            onClick={toggleTheme}
            className="p-2 rounded-full hover:bg-emerald-100 dark:hover:bg-zinc-700 transition"
            title="Toggle Theme"
          >
      ...
      {!hasSearched && (
        <div className="flex flex-col items-center justify-center flex-1 px-4 mt-10 text-center">
          <div className="mb-10">
            ...
            <p className="text-lg md:text-xl text-zinc-400 dark:text-zinc-300">
          ...
          <div className="mb-6 flex flex-wrap justify-center gap-3">
            {suggestions.map((tag) => (
              <button
                ...
                className="bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-600 text-emerald-500 dark:text-emerald-400 px-4 py-1.5 rounded-full hover:scale-105 transition-all text-sm shadow-sm"
              >
                {tag}
              </button>
            ))}
          ...
          {searchHistory.length > 0 && (
            <div className="mt-6 text-left w-full max-w-xl">
                ...
                {searchHistory.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <button
                      ...
                      className="bg-zinc-200 dark:bg-zinc-700 text-sm text-emerald-600 dark:text-emerald-400 px-3 py-1 rounded-full hover:scale-105 transition"
                    >
                      {item}
                    </button>
                    <button
                      onClick={() => removeHistoryItem(item)}
                      className="text-zinc-400 dark:text-zinc-300 hover:text-emerald-500"
              ...
              <button
                ...
                className="mt-4 text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-500"
              >
                Clear All History
              </button>
            </div>
          ...
          <div className="w-full max-w-xl relative mt-6">
            <input
              ...
              className="w-full pl-4 pr-12 py-3 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 text-black dark:text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition"
            />
            <button
              onClick={handleSearch}
              className="absolute right-5 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-emerald-400 transition"
            >
      ...
      {results.length > 0 && (
        <div className="max-w-7xl mx-auto px-6 mt-8">
          <h2 className="text-2xl font-bold mb-4">
            Search Results for <span className="text-emerald-400">"{query.charAt(0).toUpperCase() + query.slice(1)}"</span>
          </h2>
          <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {results.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className="bg-white dark:bg-zinc-800/80 backdrop-blur-md rounded-xl shadow-md overflow-hidden flex flex-col hover:shadow-lg transition-shadow border border-zinc-200 dark:border-zinc-700 relative"
                >
                  {/* Favorite Button */}
                  <button
                  ...
                  >
                    <HeartIcon
                      className={`w-5 h-5 ${isFavorited ? 'text-rose-500' : 'text-zinc-400'} transition`}
                    />
                  </button>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white dark:bg-zinc-800">
          <div className="max-w-4xl mx-auto relative">
            <input
              ...
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search for a product..."
              className="w-full pl-6 pr-12 py-3 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 text-black dark:text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition"
            />
            <button
              onClick={handleSearch}
              className="absolute right-5 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-emerald-400 transition"
